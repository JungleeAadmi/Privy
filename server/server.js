...
db.serialize(() => {
    const schemas = [
        ...
        `CREATE TABLE IF NOT EXISTS calendar_notes (id INTEGER PRIMARY KEY, date TEXT, text TEXT)`
    ];
    schemas.forEach(s => db.run(s));
    
    // --- Safe Migrations ---
    // This helper prevents the app from crashing if the column already exists
    const safeRun = (sql) => {
