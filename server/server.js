 ...
 * COMPLETE VERSION - Fully Expanded for Stability
 */
...
db.serialize(() => {
    ...
    db.run(`CREATE TABLE IF NOT EXISTS lingerie (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        filepath TEXT,
        chosen_count INTEGER DEFAULT 0
    )`);
    db.run(`CREATE TABLE IF NOT EXISTS condoms (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        filepath TEXT,
        chosen_count INTEGER DEFAULT 0
    )`);
    db.run(`CREATE TABLE IF NOT EXISTS lubes (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        filepath TEXT,
        chosen_count INTEGER DEFAULT 0
    )`);
...
app.post('/api/login', (req, res) => {
    ...
    db.get(`SELECT * FROM users WHERE username = ?`, [username], (err, user) => {
        if (!user || !bcrypt.compareSync(password, user.password)) return res.status(400).json({ error: 'Invalid' });
...
app.post('/api/settings/test', auth, (req, res) => {
    const { ntfy_url, ntfy_topic } = req.body;
    if (!ntfy_url || !ntfy_topic) return res.status(400).json({ error: 'Missing Config' });
    ...
    fetch(url, { method: 'POST', body: "Privy Test Successful! 🎉", headers: { 'Title': 'Privy Test' } })
    .then(r => {
...
app.post('/api/sections', auth, (req, res) => {
    db.run(`INSERT INTO sections (title) VALUES (?)`, [req.body.title], function(err){
        res.json({id:this.lastID});
    });
});
app.put('/api/sections/:id', auth, (req, res) => {
    db.run(`UPDATE sections SET title = ? WHERE id = ?`, [req.body.title, req.params.id], (err)=>{
        res.json({success:true});
    });
});
app.delete('/api/sections/:id', auth, (req, res) => {
    ...
    db.all(`SELECT filepath FROM cards WHERE section_id = ?`, [id], (err, rows)=>{
        ...
        db.serialize(()=>{
            ...
            db.run(`DELETE FROM sections WHERE id=?`,[id],()=>{ res.json({success:true}); });
        });
    });
});
...
app.delete('/api/cards/:id', auth, (req, res) => {
    const id=req.params.id;
    db.get(`SELECT filepath FROM cards WHERE id=?`,[id],(err,r)=>{
        ...
        db.serialize(()=>{
            ...
            db.run(`DELETE FROM cards WHERE id=?`,[id],()=>{ res.json({success:true}); });
        });
    });
});
app.post('/api/cards/:id/scratch', auth, (req, res) => {
    ...
    db.serialize(()=>{
        ...
        db.run(`UPDATE cards SET scratched_count=scratched_count+1 WHERE id=?`, [id], ()=>{
            sendNtfy(id, 'card');
            res.json({success:true});
        });
    });
...
app.post('/api/books', auth, upload.single('file'), (req, res) => {
    if(!req.file) return res.status(400).json({error:'No file'});
    db.run(`INSERT INTO books (title, filepath) VALUES (?,?)`, [req.file.originalname, `/uploads/books/${req.file.filename}`], function(err){
        res.json({id:this.lastID, filepath: `/uploads/books/${req.file.filename}`});
    });
});
app.put('/api/books/:id', auth, (req, res) => {
    db.run(`UPDATE books SET title=? WHERE id=?`, [req.body.title, req.params.id], ()=>{ res.json({success:true}); });
});
app.delete('/api/books/:id', auth, (req, res) => {
    const id=req.params.id;
    db.get(`SELECT filepath FROM books WHERE id=?`,[id],(err,r)=>{
        if(r) try{ fs.unlinkSync(path.join(DATA_DIR, 'uploads', r.filepath.replace('/uploads/',''))); }catch(e){}
        db.run(`DELETE FROM books WHERE id=?`,[id],()=>{ res.json({success:true}); });
    });
});
...
// Gallery Handlers (Toys, Lingerie, Protection)
