...
db.serialize(() => {
    ...
    db.run(`CREATE TABLE IF NOT EXISTS header_sections (id INTEGER PRIMARY KEY, title TEXT)`);
    db.run(`CREATE TABLE IF NOT EXISTS sections (id INTEGER PRIMARY KEY, title TEXT, header_id INTEGER)`);
    db.run(`CREATE TABLE IF NOT EXISTS cards (id INTEGER PRIMARY KEY, filepath TEXT, scratched_count INTEGER DEFAULT 0, section_id INTEGER)`);
    db.run(`CREATE TABLE IF NOT EXISTS card_history (id INTEGER PRIMARY KEY, card_id INTEGER, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(card_id) REFERENCES cards(id))`);
    db.run(`CREATE TABLE IF NOT EXISTS books (id INTEGER PRIMARY KEY, title TEXT, filepath TEXT)`);
    db.run(`CREATE TABLE IF NOT EXISTS dice_options (id INTEGER PRIMARY KEY, type TEXT, text TEXT, role TEXT DEFAULT 'wife')`);
    db.run(`CREATE TABLE IF NOT EXISTS location_unlocks (id INTEGER PRIMARY KEY, name TEXT UNIQUE, unlocked_at DATETIME, count INTEGER DEFAULT 0)`);
    db.run(`CREATE TABLE IF NOT EXISTS fantasies (id INTEGER PRIMARY KEY, text TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, pulled_at DATETIME)`);
    
    // Galleries
    db.run(`CREATE TABLE IF NOT EXISTS toys (id INTEGER PRIMARY KEY, filepath TEXT, chosen_count INTEGER DEFAULT 0)`);
    db.run(`CREATE TABLE IF NOT EXISTS lingerie (id INTEGER PRIMARY KEY, filepath TEXT, chosen_count INTEGER DEFAULT 0)`);
    ...
    // Calendar
    db.run(`CREATE TABLE IF NOT EXISTS calendar_notes (id INTEGER PRIMARY KEY, date TEXT, text TEXT)`);

    // 2. Migrations (Safe updates)
    const safeRun = (sql) => { try { db.run(sql); } catch (e) {} };
    safeRun(`ALTER TABLE dice_options ADD COLUMN role TEXT DEFAULT 'wife'`);
    safeRun(`ALTER TABLE location_unlocks ADD COLUMN count INTEGER DEFAULT 0`);
    safeRun(`ALTER TABLE sections ADD COLUMN header_id INTEGER`);
});

// --- Ntfy Helper ---
const sendNtfy = (id, type) => {
    db.all(`SELECT * FROM settings`, [], (err, rows) => {
        ...
        db.get(`SELECT filepath FROM ${table} WHERE id = ?`, [id], (err, row) => {
            if (!row) return;
...
const auth = (req, res, next) => {
    ...
    try {
        req.user = jwt.verify(t.split(' ')[1], SECRET_KEY);
        next();
...
app.put('/api/user', auth, (req, res) => {
    ...
    if (req.body.password) {
        q += `, password=?`;
        p.push(bcrypt.hashSync(req.body.password, 8));
    }
    q += ` WHERE id=?`;
    p.push(req.user.id);
...
app.delete('/api/headers/:id', auth, (req, res) => {
    db.serialize(() => {
        ...
        db.run(`DELETE FROM header_sections WHERE id=?`, [req.params.id]);
        res.json({ success: true });
    });
});
...
app.get('/api/export', auth, (req, res) => {
    ...
    db.serialize(() => {
        ...
        setTimeout(() => {
            ...
            // Use path.basename to ensure zip command stays inside the temp dir
