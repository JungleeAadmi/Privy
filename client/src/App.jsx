    ...
    return (
        <div className="p-4 pb-24 min-h-screen flex flex-col items-center w-full">
            ...
            {editing && <div className="w-full grid grid-cols-3 gap-2">
                ...
                {items.map(i => {
                    const lp = useLongPress(()=>setDelId(i.id));
                    return <div key={i.id} {...lp} className="aspect-square bg-gray-900 rounded overflow-hidden relative"><img src={i.filepath} className="w-full h-full object-cover"/></div>
                })}
            </div>}
